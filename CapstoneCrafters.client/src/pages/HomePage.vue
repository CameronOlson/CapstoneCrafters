<template>
  <div class="row bg-one">
    <div class="col-12">
      <h1 class="my-5 p-3 text-center">Welcome to Capstone Crafters</h1>
    </div>
    <div class="col-6">
      <div class="ms-5">
        <h1>Mission Statement</h1>
        <p>
          Lorem ipsum dolor sit amet consectetur adipisicing elit. Quod ipsam
          magni nobis cumque illo, ducimus corporis minima maiores adipisci
          praesentium nemo fugiat ut ratione eveniet cupiditate nesciunt
          eligendi? Esse aliquam maiores odio, consectetur iusto quos debitis,
          voluptatem explicabo corrupti officia dolorum nesciunt. Delectus omnis
          reprehenderit consequatur impedit aliquid rerum ea possimus pariatur
          consequuntur quibusdam! At ut eaque, vitae ullam rerum maxime esse
          repellendus aliquam, vel quod, quas molestiae fuga fugiat modi
          exercitationem corporis. Facere tempore dolore fuga provident eveniet
          totam! Tenetur autem possimus reiciendis dolorem blanditiis,
          dignissimos, consequatur doloremque voluptatum repudiandae
          necessitatibus assumenda cum accusantium temporibus expedita. Optio,
          pariatur dolorem? <br />
          <br />
          Esse aliquam maiores odio, consectetur iusto quos debitis, voluptatem
          explicabo corrupti officia dolorum nesciunt. Delectus omnis
          reprehenderit consequatur impedit aliquid rerum ea possimus pariatur
          consequuntur quibusdam! At ut eaque, vitae ullam rerum maxime esse
          repellendus aliquam, vel quod, quas molestiae fuga fugiat modi
          exercitationem corporis. <br />
          <br />
          Lorem ipsum dolor sit amet consectetur adipisicing elit. Quod ipsam
          magni nobis cumque illo, ducimus corporis minima maiores adipisci
          praesentium nemo fugiat ut ratione eveniet cupiditate nesciunt
          eligendi? Esse aliquam maiores odio, consectetur iusto quos debitis,
          voluptatem explicabo corrupti officia dolorum nesciunt. Delectus omnis
          reprehenderit consequatur impedit aliquid rerum ea possimus pariatur
          consequuntur quibusdam! At ut eaque, vitae ullam rerum maxime esse
          repellendus aliquam, vel quod, quas molestiae fuga fugiat modi
          exercitationem corporis. Facere tempore dolore fuga provident eveniet
          totam! Tenetur autem possimus reiciendis dolorem blanditiis,
          dignissimos, consequatur doloremque voluptatum repudiandae
          necessitatibus assumenda cum accusantium temporibus expedita. Optio,
          pariatur dolorem?
        </p>
      </div>
    </div>
    <div class="col-6">
      <div class="row m-0 p-0">
        <div class="col-12 d-flex standard">
          <div class="text-center">
            <p>Creator 1</p>
            <router-link
              :to="{ name: 'Account1' }"
              class="btn text-success lighten-30 selectable text-uppercase"
            >
              <img
                class="thumbnail"
                src="https://i.natgeofe.com/n/46b07b5e-1264-42e1-ae4b-8a021226e2d0/domestic-cat_thumb_square.jpg"
                width="500"
                height="600"
              />
            </router-link>
          </div>

          <div class="text-center">
            <p>Creator 2</p>
            <img
              class="thumbnail"
              src="data:image/jpeg;base64,"
              width="500"
              height="600"
            />
          </div>
          <div class="text-center">
            <p>Creator 3</p>
            <img
              class="thumbnail"
              src="https://cdn.britannica.com/39/7139-050-A88818BB/Himalayan-chocolate-point.jpg"
              width="500"
              height="600"
            />
          </div>
          <div class="text-center">
            <p>Creator 4</p>
            <img
              class="thumbnail"
              src="https://www.rd.com/wp-content/uploads/2021/01/GettyImages-1175550351.jpg"
              width="500"
              height="600"
            />
          </div>
        </div>
        <div class="col-1"></div>
        <div class="col-10 mt-5">
          <div class="card img-fluid rounded">
            <img
              src="https://media.newyorker.com/photos/590971712179605b11ad7a88/4:3/w_1787,h_1340,c_limit/Jabr-AreCatsDomesticated.jpg"
              style="width: 100%"
            />
            <div class="card-img-overlay">
              <h4 class="card-title text-center">Project 1</h4>
              <span class="text-bottom p-1 m-1 text-light">
                <p>
                  Lorem, ipsum dolor sit amet consectetur adipisicing elit. Ad
                  blanditiis eos aperiam, hic vel iusto ullam. Voluptate ad
                  harum mollitia eos deleniti, ea repellendus amet id molestias?
                  Magnam doloribus voluptas nam ab. Debitis sunt maxime
                  perspiciatis quod laborum deserunt, repellendus assumenda
                  autem distinctio cumque molestiae dignissimos nostrum fuga
                  illum nisi?
                </p>
              </span>
            </div>
          </div>
        </div>
        <div class="col-1"></div>
      </div>
    </div>
  </div>
  <div class="row bg-dark">
    <div class="col-6">
      <div class="row">
        <div class="col-1"></div>
        <div class="col-10 mt-5">
          <div class="card img-fluid">
            <img
              src="https://cattalesinc.org/wp-content/uploads/slider2/DSC_0029-scaled.jpeg"
              style="width: 100%"
            />
            <div class="card-img-overlay">
              <h4 class="card-title text-center">Project 2</h4>
              <span class="text-bottom p-1 m-1 text-light">
                <p>
                  Lorem, ipsum dolor sit amet consectetur adipisicing elit. Ad
                  blanditiis eos aperiam, hic vel iusto ullam. Voluptate ad
                  harum mollitia eos deleniti, ea repellendus amet id molestias?
                  Magnam doloribus voluptas nam ab. Debitis sunt maxime
                  perspiciatis quod laborum deserunt, repellendus assumenda
                  autem distinctio cumque molestiae dignissimos nostrum fuga
                  illum nisi?
                </p>
              </span>
            </div>
          </div>
        </div>
        <div class="col-1"></div>
      </div>
    </div>
    <div class="col-6 pt-5">
      <h1>Somthing Else That is Important</h1>
      <p>
        Esse aliquam maiores odio, consectetur iusto quos debitis, voluptatem
        explicabo corrupti officia dolorum nesciunt. Delectus omnis
        reprehenderit consequatur impedit aliquid rerum ea possimus pariatur
        consequuntur quibusdam! At ut eaque, vitae ullam rerum maxime esse
        repellendus aliquam, vel quod, quas molestiae fuga fugiat modi
        exercitationem corporis. <br />
        <br />
        Lorem ipsum dolor sit amet consectetur adipisicing elit. Quod ipsam
        magni nobis cumque illo, ducimus corporis minima maiores adipisci
        praesentium nemo fugiat ut ratione eveniet cupiditate nesciunt eligendi?
        Esse aliquam maiores odio, consectetur iusto quos debitis, voluptatem
        explicabo corrupti officia dolorum nesciunt. Delectus omnis
        reprehenderit consequatur impedit aliquid rerum ea possimus pariatur
        consequuntur quibusdam! At ut eaque, vitae ullam rerum maxime esse
        repellendus aliquam, vel quod, quas molestiae fuga fugiat modi
        exercitationem corporis. Facere tempore dolore fuga provident eveniet
        totam! Tenetur autem possimus reiciendis dolorem blanditiis,
        dignissimos, consequatur doloremque voluptatum repudiandae
        necessitatibus assumenda cum accusantium temporibus expedita. Optio,
        pariatur dolorem?
      </p>
    </div>
  </div>
  <div class="row bg-one">
    <div class="col-5 ms-5">
      <h1>About Us</h1>
      <p>
        Esse aliquam maiores odio, consectetur iusto quos debitis, voluptatem
        explicabo corrupti officia dolorum nesciunt. Delectus omnis
        reprehenderit consequatur impedit aliquid rerum ea possimus pariatur
        consequuntur quibusdam! At ut eaque, vitae ullam rerum maxime esse
        repellendus aliquam, vel quod, quas molestiae fuga fugiat modi
        exercitationem corporis. <br />
        <br />
        Lorem ipsum dolor sit amet consectetur adipisicing elit. Quod ipsam
        magni nobis cumque illo, ducimus corporis minima maiores adipisci
        praesentium nemo fugiat ut ratione eveniet cupiditate nesciunt eligendi?
        Esse aliquam maiores odio, consectetur iusto quos debitis, voluptatem
        explicabo corrupti officia dolorum nesciunt. Delectus omnis
        reprehenderit consequatur impedit aliquid rerum ea possimus pariatur
        consequuntur quibusdam! At ut eaque, vitae ullam rerum maxime esse
        repellendus aliquam, vel quod, quas molestiae fuga fugiat modi
        exercitationem corporis. Facere tempore dolore fuga provident eveniet
        totam! Tenetur autem possimus reiciendis dolorem blanditiis,
        dignissimos, consequatur doloremque voluptatum repudiandae
        necessitatibus assumenda cum accusantium temporibus expedita. Optio,
        pariatur dolorem?
      </p>
    </div>
    <div class="col-6">
      <h1>Contact Us</h1>
      <form>
        <div class="form-group">
          <label for="exampleFormControlInput1">Email address</label>
          <input
            type="email"
            class="form-control"
            id="exampleFormControlInput1"
            placeholder="name@example.com"
          />
        </div>
        <div class="form-group">
          <label for="exampleFormControlTextarea1">Write Your Message</label>
          <textarea
            class="form-control"
            id="exampleFormControlTextarea1"
            rows="3"
          ></textarea>
        </div>
        <div>
          <button class="btn-primary">Submit</button>
        </div>
      </form>
    </div>
  </div>
</template>

<script>
export default {
  name: 'Home'
}
</script>

<style scoped lang="scss">
.home {
  display: grid;
  height: 80vh;
  place-content: center;
  text-align: center;
  user-select: none;
  .home-card {
    width: 50vw;
    > img {
      height: 200px;
      max-width: 200px;
      width: 100%;
      object-fit: contain;
      object-position: center;
    }
  }
}
.bg-one {
  background-color: rgb(43, 143, 179);
}
.thumbnail {
  height: 8rem;
  width: 8rem;
  border-radius: 50%;
}
.standard {
  justify-content: space-around;
}
.center-these {
  justify-content: center;
}
.text-bottom {
  position: absolute;
  bottom: 0;
  right: 0;
}
.rounded {
  box-shadow: 2px 5px;
  border-radius: 10px;
}
</style>
